...
def country_thisMonth(df):
    ...
    nameStr = ""
    for dummy_index, row in good_water_decommissioned_sites.iterrows():
    ...
    good_water_scale = round((good_water_count[1] / good_water_sum), 5)
    ...
    expected_water_scale = round(
        (expected_water_count[1] / good_water_sum), 5)
    # print("合格比例：" + str(expected_water_scale))

    # 未达标统计below quality
    below_quality_sites = good_water_df[good_water_df["合格"] == 0]
    below_quality_name = ""
    for dummy_index, row in below_quality_sites.iterrows():
    ...
    result += "---------------------------------------------------" + seq
    result += "优Ⅲ统计：" + seq
    result += "总数：" + str(good_water_sum) + " 优Ⅲ：" + \
        str(good_water_count[1]) + " 否：" + str(good_water_count[0]) + seq
    result += "优Ⅲ比例：" + str(good_water_scale) + seq
    result += "---------------------------------------------------" + seq
    result += "合格统计：" + seq
    result += "总数：" + str(good_water_sum) + " 合格：" + str(
        expected_water_count[1]) + " 否：" + str(expected_water_count[0]) + seq
